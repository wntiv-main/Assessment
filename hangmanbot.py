...
from discord.ext.commands import Cooldown, CooldownMapping, BucketType, slash_command
...
class HangmanBot(Bot):
    ...
    def _on_new_config(self, guild: str, name: str, config: cfg.GamemodeConfig):
    ...
    def run(self) -> None:
        super().run(
            self.config.get_value(cfg.BotConfig.DISCORD_TOKEN), reconnect=True
        )
    ...
    def _get_gamemodes(ctx: AutocompleteContext):
        ...
        return self.gamemodes_manager.gamemodes[str(ctx.interaction.guild_id)].keys()

    @staticmethod
    @slash_command()
    async def play(
        ctx: ApplicationContext, gamemode: Option(SlashCommandOptionType.string, autocomplete=basic_autocomplete(_get_gamemodes), required=True)
    ):
        gamemodes: dict[str, cfg.GamemodeConfig] = ctx.bot.gamemodes_manager.gamemodes[str(ctx.guild_id)]
        if gamemode not in gamemodes:
            ctx.send(
                embed=Embed(
                    title="Invalid option!",
                    description="That gamemode doesn't exist (yet). Please try \
                    again, or if you think that this is and error, contact an \
                    administrator. Valid options are:",
                    color=Color.from_rgb(255, 0, 0),
                    fields=[
                        EmbedField(
                            name=name,
                            value=config.get_value(cfg.GamemodeConfig.DESCRIPTION)
                        )
            ...
            game = config.get_value(config.GamemodeConfig.GAME_TYPE).value(config)
            game.run(ctx)
